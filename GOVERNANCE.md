...
This document describes how the **decroche-mcp** project is governed and how decisions are made.
...
- Routine changes (bug fixes, docs, dependency bumps): one maintainer approval + green CI is enough
  to merge.
- Significant changes (new features, API/behavior changes, architecture): open a PR and/or an
  **ADR** (see `docs/adr/`). If no maintainer objects within **7 days**, it is approved.
...
release-please (see [release reference](./CHANGELOG.md)). Any maintainer may approve and merge the
release PR.
